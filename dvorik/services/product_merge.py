    ...
    *,
    ...
    with conn:
        ...
        return {
            ...
            "article_aliases": alias_records,
            "name_aliases": name_alias_records,
        }


def undo_merge(conn: sqlite3.Connection, merge_id: int) -> Dict[str, Any]:
    row = conn.execute("SELECT * FROM product_merge_log WHERE id=?", (merge_id,)).fetchone()
    if not row:
        return {"ok": False, "error": "Запись объединения не найдена"}
    if row["reverted_at"]:
        return {"ok": False, "error": "Объединение уже отменено"}

    changes = json.loads(row["changes_json"] or "{}")
    base_id = int(row["source_a_id"])
    other_id = int(row["source_b_id"])
