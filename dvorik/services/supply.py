...
from dvorik.core import constants as const
from dvorik.db import db
from dvorik.core.constants import DEFAULT_SUPPLIER_NAME
from dvorik.utils.number import to_float_qty
from dvorik.services import product_merge as merge_svc

# Compatibility helpers previously provided by the legacy app layer.
def log_event_to_skl(conn: sqlite3.Connection, pid: int, loc: str, delta: float) -> None:
    """Persist a simplified event log entry for stock moves to the hub."""
    ...
    with conn:
        conn.execute(
            """
            INSERT INTO event_log(event_type, product_id, location_code, delta)
            VALUES ('to_skl', ?, ?, ?)
            """,
            (pid, loc, float(delta)),
        )
...
# Historically imports updated restock metadata in place.
def mark_restock(conn: sqlite3.Connection, pid: int) -> None:
    """Update restock timestamp and unarchive the product if needed."""
    ...
    conn.execute(
        ...
        UPDATE product
        SET last_restock_at=?,
            archived=CASE WHEN archived=1 THEN 0 ELSE archived END,
...
def write_normalized_csv(rows: List[Tuple[str, str, float]], base_name: str) -> str:
    ...
    out_path = config.normalized_uploads_dir / f"{safe_base}.csv"
